...
\begin{equation}
P(X = \frac{1}{2}) = P\left(X = \frac{1}{2}, 0 \leq Y \leq  1\right) = \int_{0}^{1} \int_{\frac{1}{2}}^{\frac{1}{2}} (x + 2y^3) \, dx \, dy = 0
\end{equation}



So, the probability \( P(X = \frac{1}{2}) \) is 0.
...
Thus, in order to solve the problem, we should use binomial cdf function:

\[
P(X \geq 90) = \sum_{k=90}^{1000} P(X = k) = 1 - P(X \leq 89)
\]

Since calculating this directly can be complex, let's calculate it using Octave.

\begin{figure}[H]
    \centering
    \includegraphics[scale=0.5]{q3a}
    \caption{The MATLAB code of binomial cdf for n = 1000}
\end{figure}

Therefore the probability that at least 9$\%$ of the soldiers belong to the naval forces is approximately 0.8666.
...
\begin{figure}[H]
    ...
    \caption{The MATLAB code of binomial cdf for n = 2000}
\end{figure}

In this case, the probability that at least 9$\%$ of the soldiers belong to the naval forces is 0.9385.\\

If the retinue is increased to 2000 soldiers while maintaining the same minimum percentage requirement (9$\%$ of 2000 soldiers = 180), the probability of having at least that many naval soldiers will increase. Because of Central Limit Theorem, as the sample size increases, the distribution of sample means tends to resemble a normal distribution.\\
...
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.6]{q4b}
    \caption{The MATLAB code of Distribution Histograms}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[scale=0.5]{q4b1}
    \caption{Distribution Histogram with random sample of size 20}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[scale=0.5]{q4b2}   
    \caption{Distribution Histogram with random sample of size 100}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[scale=0.5]{q4b3}
    \caption{Distribution Histogram with random sample of size 1000}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{The MATLAB code of 1000 iterations for random sample}
\end{figure}

In part (a), the probability that a randomly selected elephant will live more than 60 years but less than 75 years was calculated to be approximately 0.7583. This implies that in a large population of elephants, around 75.83$\%$ of them are expected to fall within this age range.

In part (c), through simulation, random samples of 100 elephants' lifespans were generated, and the count was made for how many of these samples had at least 70$\%$ of the elephants falling within the specified age range (60 to 75 years). It was found that out of 1000 simulations, 891 of them met this criterion. This result aligns with the expectation from part (a) because the probability of having at least 70$\%$ of the elephants within the specified range is close to the calculated probability of 0.7583.

However, when looking at the criterion of having at least 85$\%$ of the elephants within the specified age range, it was found that only 9 out of 1000 simulations met this criterion. This result is also consistent with the expectation from part (a). Since the probability of having 85$\%$ of the elephants within the specified range is higher than the probability of having 70$\%$ within the same range, it is less likely to observe a large number of simulations meeting this criterion. Hence, the small number of simulations (9 out of 1000) aligns with the lower probability calculated for this criterion.

In summary, the simulation results reflect the probabilities calculated in part (a), showing that simulations tend to align with theoretical probabilities based on the normal distribution model.
